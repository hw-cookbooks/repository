def load_current_resource
  ...
  @architecture = `dpkg-deb -f #{new_resource.path} Architecture`.strip
end
...
action :add do
  ...
  (p_arch == 'all' ? archs : (archs & Array(p_arch))).each do |arch|
    next if node.run_state[:repository_db][codename][:components][component][:architectures][arch].include?(pool_file)
